...
function setupMsgScrollHandler() {
  var msgWindow = $j('.col-md-8 .list-group.pre-scrollable');
  var msgList = $j('#messages-list');
...
function onDeliveredStatusListener(messageId) {
  $j('#delivered_'+messageId).fadeIn(200);
}

function onReadStatusListener(messageId) {
  $j('#delivered_'+messageId).fadeOut(100);
  $j('#read_'+messageId).fadeIn(200);
}
...
function retrieveChatMessages(dialog, beforeDateSent){
  ...
  QB.chat.message.list(params, function(err, messages) {
    if (messages) {
      ...
      if(messages.items.length === 0) {
        $j("#no-messages-label").removeClass('hide');
      } else {
        $j("#no-messages-label").addClass('hide');
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          $j('#messages-list').prepend(messageHtml);
          ...
          if (item.read_ids.length > 1 && messageSenderId === currentUser.id) {
            ...
            $j('#delivered_'+messageId).fadeIn(100);
            $j('#read_'+messageId).fadeOut(200);
          }

          if (i > 5) {$j('#messages-list').scrollTop($j('#messages-list').prop('scrollHeight'));}
        });
      }
    }else{
      console.log(err);
  ...
  $j(".load-msg").delay(100).fadeOut(500);
}


// sending messages after confirmation
function clickSendMessage() {
  var currentText = $j('#message_text').val().trim();
  if (currentText.length === 0){
    return;
  }

  $j('#message_text').val('').focus();
...
function sendMessage(text, attachmentFileId) {
  ...
  if (currentDialog.type === 3) {
    ...
    $j('.list-group-item.active .list-group-item-text').text(msg.body);
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  $j('#messages-list').append(messageHtml);

  // scroll to bottom
  var mydiv = $j('#messages-list');
...
function setupIsTypingHandler() {
  ...
  $j("#message_text").focus().keyup(function(){
...
function showUserIsTypingView(isTyping, userId, dialogId) {
  if(isMessageForCurrentDialog(userId, dialogId)){
    ...
    if (!isTyping) {
      $j('#'+userId+'_typing').remove();
    } else if (userId != currentUser.id) {
      var userLogin = getUserLoginById(userId);
      var typingUserHtml = buildTypingUserHtml(userId, userLogin);
      $j('#messages-list').append(typingUserHtml);
    }
