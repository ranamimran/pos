/**
 * Created by Muhammad.Imran on 1/7/2016.
 */
...
var stepcarousel={
	...
	var baseurl=$('#baseurl').val();
        config.$leftnavbutton=$('<img src="'+ baseurl + '/orders/'+config.defaultbuttons.leftnav[0]+'" class="' + config.galleryid + '_navbutton">').css({zIndex:50, position:'absolute', left:config.offsets.left+config.defaultbuttons.leftnav[1]+'px', top:config.offsets.top+config.defaultbuttons.leftnav[2]+'px', cursor:'hand', cursor:'pointer'}).attr({title:'Back '+config.defaultbuttons.moveby+' panels'}).appendTo('body')
        config.$rightnavbutton=$('<img src="'+ baseurl + '/orders/'+config.defaultbuttons.rightnav[0]+'" class="'  + config.galleryid + '_navbutton">').css({zIndex:50, position:'absolute', left:config.offsets.left+config.$gallery.get(0).offsetWidth+config.defaultbuttons.rightnav[1]+'px', top:config.offsets.top+config.defaultbuttons.rightnav[2]+'px', cursor:'hand', cursor:'pointer'}).attr({title:'Forward '+config.defaultbuttons.moveby+' panels'}).appendTo('body')
