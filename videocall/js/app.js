    ...
    $(function() {
        ...
        function initializeUI(arg) {
            ...
            QB.users.listUsers(params, function(err, result){
                if (result) {
                    var QBUserss=[];          
                    var dataa = result.items;
                    if(dataa.length>0)
                    {
                        for(var i=0; i<dataa.length; i++)
                        {
                            var currentTimestamp = Math.floor(Date.now() / 1000);
                            var userLastRequestAtTimestamp = Math.floor(new Date(dataa[i].user.last_request_at) / 1000);
                            // if user didn't do anything last 5 minutes (5*60 seconds)    
                            if((currentTimestamp - userLastRequestAtTimestamp) > 5*60){ 
                                 QBUserss[i]=dataa[i].user;
                                 QBUserss[i]['status']='yes';
                            }else
                            {
                                QBUserss[i]=dataa[i].user;
                                QBUserss[i]['status']='no';
                            }
                ...
                //console.log("QBUsers: " + JSON.stringify(QBUsers));
            ...
            {
                var param1 = parseInt(user_id);
                QB.users.get(param1, function(err, user){
                if (user) {
                    var QBUserss=[];          
                        var currentTimestamp = Math.floor(Date.now() / 1000);
                        var userLastRequestAtTimestamp = Math.floor(new Date(user.last_request_at) / 1000);
                        // if user didn't do anything last 5 minutes (5*60 seconds)    
                        if((currentTimestamp - userLastRequestAtTimestamp) > 5*60){ 
                             QBUserss=user;
                             QBUserss['status']='yes';
...
//                }
        ...
        function loginUser(){
            ...
            app.caller = {
                id: +$.trim( localStorage.getItem("qb_admin_id") ),
                login: $.trim( localStorage.getItem("qb_login") ),
                password: $.trim( localStorage.getItem("qb_password") ),
                full_name: $.trim("Admin")
            };

            var usersWithoutCaller = _.filter(QBUsers, function(i) { return i.id !== app.caller.id; });
